# React → Vue 平替：挂账总清单（截至 wave 91，2026-09-06）

这份文件回答一个问题：**「还欠什么」。** 逐条给状态，不给散文。
深度背景在 `vue-parity-handoff.md`，踩坑线索在 Claude 记忆 `deerflow-parity-harness-plan`。

> **判据提醒**：台账现在是 **2 行 / 51 个取样点**（wave 78 清零后第一次有意接受，
> 两行都在下面第一节里逐条交代）。「0 行」的准确含义一直是「**这些取样点上量不出差异**」，
...
e2e-parity       59        台账 2 行 / 51 样本（两行见第一节第 3 条）
...
> **这条尾巴没有自然终点。** 历史命中率：wave 91 给尺子加两档、一次量出 17 行

...
2. Stream events update thread state (messages, artifacts, todos, goal). The main thread stream uses the LangGraph SDK's `throttle: true` mode so updates received in the same macrotask coalesce before React is notified; do not replace it with a numeric delay without validating the SDK's trailing-debounce behavior on a continuous stream.
   File-tool artifact auto-open work must run in an effect with timer cleanup; never schedule timers while rendering streamed `write_file`, `str_replace`, or staged artifact write updates. `write_file`/`str_replace` may open a streamed draft URL while loading; `finalize_artifact_write` opens the real artifact path only after the tool returns `OK`.
   HTML write-file previews must pass the same lightweight document sanity check used by `core/artifacts/preview.ts`: a completed `.html`/`.htm` draft with missing or out-of-order `html`/`body` tags, or mismatched `head`/`style`/`script` tags when those regions appear, stays in code view instead of being rendered in an iframe. Prefix chunks are allowed while a document is still being assembled; finalized HTML artifacts are validated backend-side for structure and local render-resource existence before the target file is written.

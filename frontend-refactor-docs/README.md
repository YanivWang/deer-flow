...
> | | 变更 |
> | --- | --- |
> | 🔴 **D26** | **上游冻结政策**（新，✅ **用户已定案 (b)**）—— 只 merge `backend/` + 安全补丁，`frontend/` 冻结。实测近 6 个月 `frontend/src` **文件级 100% 翻动**（453 唯一文件 / 389 现存），风险 **R6 由「中/低」上调为「必然/高」** |
> | 🔴 **D25-a** | `crypto.randomUUID()` **secure-context-only**，内网 HTTP 下 `undefined` → **新建会话直接报错**；`@vueuse/core` 改回保留 |
> | 🔴 **D13-a** | 补第六类偏离 **DETYPED**（13 文件 / 3,185 行）→ **逐字节复制 10,400 → 7,215 行** |

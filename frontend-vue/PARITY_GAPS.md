...
| 2026-08-25 | 测试目录去迁移化                                              | `make verify` 194 files / 1596 tests；`make e2e-mock` 240、`make e2e-backend` 21、`make e2e-visual` 8/8、`make asset-budget` 全绿；`tests/unit/` 的 12 个阶段目录（`wp02`…`wp12`、`m7`）共 69 个测试文件按用途归位，24 个文件的 `describe("WP-xx …")` / `M7` 前缀清空；`doc-facts.test.ts` 新增 2 条并逐条变异验证：新建阶段命名目录会红、`describe` 标题重新加阶段前缀会红                                                                                                                                   | 只是改名，没有删测试。剩下的阶段标识（`app/pages/__m0/`、`NUXT_PUBLIC_M0_TEST_PAGES`、`tests/support/run_m0_gateway.py`、`m0_replay_provider.py`、`AGENT_CORE_CONTRACT_VERSION = "m8"`）是**标识符**不是措辞，改它们会波及 Dockerfile、CI 与后端 harness，已在 `ARCHITECTURE.md` 显式列出并由门禁双向冻结。顺手修掉 Makefile 自称「M0 developer entrypoint」和 `proxy-probe.mjs` 指向已删除 config 两处死指针。提交 `d1522cc8`；仅改 `frontend-vue/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 2026-08-25 | 移除 Pinia                                                    | `make verify` 194 files / 1596 tests；`make e2e-mock` 240、`make e2e-backend` 21、`make e2e-visual` 8/8（基线无变化）、`make asset-budget` 全绿；生产包里 `name:"pinia"`、`$pinia`、`createPinia` 三个特征串各由 1 降为 0；`vendor-vue` raw 367,164 → 361,396、gzip 78,508 → 76,555                                                                                                                                                                                                                           | `pinia` 与 `@pinia/nuxt` 是已声明依赖且模块已在 `nuxt.config.ts` 注册，而工作区里 `defineStore` 数量为 0——它不是无害的未用依赖，它的 plugin 进了生产包，每次加载都往 `globalProperties` 挂一个没人读的 `$pinia`。文档里 23 处「Pinia 不复制 server state」改成不带品牌名的同义约束；`packages/agent-core/tests/architecture.test.ts` 仍禁止 import `pinia`，那是禁令不是清单。提交 `a1017056`；仅改 `frontend-vue/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 2026-08-25 | KaTeX 按需加载 + 路由首屏预算                                 | `make verify` 196 files / 1612 tests；`make e2e-mock` 245、`make e2e-backend` 21、`make e2e-visual` 8/8（基线无变化）、`make asset-budget` 全绿；工作区首屏 1,800,744 → 1,531,995 B（−268,749，−14.9%），含公式内容按需加载后回到 1,801,709；新增 `tests/e2e/route-payload.spec.ts` + `baseline/route-payload-budget.json`，实测 `/` 46 files / 733,848 raw / 221,025 brotli、`/login` 46 / 741,071 / 223,314、`/workspace/chats/new` 56 / 1,531,995 / 424,662                                                | `rehype-katex` 原本在 `app/core/markdown/plugins.ts` 顶层 import，264 KB 无条件进每次工作区加载；`math.ts` 的 `containsMath` + 缓存动态 import 收口该 seam，判据刻意宁可多报不可漏报（漏报是公式永远不渲染且不报错）。`make asset-budget` 对这类回归结构性失明：本次首屏少 269 KB，而它的总数**涨了 1 KiB**。关键路径禁入用**库本体**特征串识别，不能按包名——动态 import 的引用会以包名形式出现在同步 chunk 里。一条实测陷阱：组件测试不静态 import `StreamMarkdown.vue` 时 `wrapper.html()` 恒为空串，全部 `not.toContain` 假绿。提交 `129ef15f`；仅改 `frontend-vue/`                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 2026-08-25 | 首屏预算分组 + zod / parse5 下沉                              | `make verify` 196 files / 1612 tests；`make e2e-mock` 5 套件 245、`make e2e-auth` 18/18、`make e2e-visual` 8/8（基线无变化，含 `artifact panel`）、`make asset-budget` 全绿；route-payload 5/5 并按 critical / prefetch 两组分别钉死。关键路径 brotli：`/` 136,437 → 124,974、`/login` 127,681 → 116,121、`/workspace/chats/new` 329,029 → 276,787；工作区 critical+prefetch 合计 424,746 → 386,350（−38,396，真减少而非搬进 prefetch）                                                                       | 三件事。①旧预算把 `rel=prefetch` 算成首屏：`/` 那 4 个 reka-ui chunk（65,015 brotli）实测 100% 是 `VeryLow` 投机下载，阻塞首屏的 136 KB 里没有一个字节 reka-ui；现按 CDP `initialPriority` 分组。②全局 middleware 静态 import `session-query` → `auth/types`，把整个 zod（57,358 raw）放进**每个**公开页；改成分支内 `await import()`，但 `useQueryClient()` 必须在 `await` 之前取，否则丢 Nuxt 异步上下文，`make e2e-auth` 12 条红 10 条。③`vendor-markdown` 名不副实：工作区那 172,985 brotli 里 79,938 是 lucide/reka/agent-core，真正的 markdown 库是 93,047；其中 `parse5` 占 chunk 源码 24.8%，只经 `ArtifactPreview` 的 `rehype-raw` 进来，而消息路径从不渲染 raw HTML——`ArtifactPanel` 改 `defineAsyncComponent`。i18n 按 locale 分包实测**净多下载 2,992 brotli**（Nuxt 给 entry 的每个动态 import 都发 prefetch，两本词典都会被拉；当前那本还拿不到 `modulepreload`），已按证据放弃并写进 `ARCHITECTURE.md`。提交 `d84de757`、`d5fd2e3a`、`ea5f7a54`、`0f94d5a1`；仅改 `frontend-vue/` |

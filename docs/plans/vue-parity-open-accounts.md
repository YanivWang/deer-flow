# React → Vue 平替：挂账总清单（截至 wave 99，2026-09-06）
...
> **方向 B 的存量也快见底了**：`opacity`（wave 91）、`focus`（94）、`order`（95）、
> tab 序（96）都补完了；**层级那一半 wave 99 量完判定不必做**。
> 名义上还没覆盖的只剩 `pointer-events` 与 `z-index` 实际堆叠，
> 加之前先按坑 258 问一句：**有没有一种变异能让它响、而现有的档都不响？**（`tests/guards/` 下已有十条 + wave 83 加进
...
> **这条尾巴没有自然终点。** 历史命中率：wave 97 结清 tab 序那四处、

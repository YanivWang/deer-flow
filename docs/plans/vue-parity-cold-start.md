...
你接手一个已经跑了 **125 轮**的长期任务：把 `frontend-vue/`（Nuxt/Vue）对齐
...
2. `docs/plans/vue-parity-handoff.md` —— **轮次交接文档**，约 4100 行。
   ...
   **最近二十五轮（101~125）在最前面，先读它们**——这个阶段的方法论都在那里。
...
- **默认只改 `frontend-vue/`。** 例外只有一种：**上游自己是坏的**——
  ...
  （`make -C frontend-vue upstream-accept`）。**动过 `frontend/` 的至今是二十二轮**（wave 106~125 都没动），
...
```bash
make -C <abs>/frontend-vue verify          # exit 0；265 文件 / 2204 单测；词典 942 key / 18 unused

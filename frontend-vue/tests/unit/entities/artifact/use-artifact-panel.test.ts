...
import {
  ...
  useArtifactPanel,
} from "../../../../app/entities/artifact/use-artifact-panel";
...
describe("useArtifactPanel", () => {
  ...
  it("auto-opens only when a new artifact is discovered during an active run", async () => {
    const routePath = ref("/workspace/chats/thread-a");
    const discovered = ref<readonly unknown[]>([]);
    const isBusy = ref(false);
    const wrapper = mountArtifactHarness(routePath, discovered, isBusy);

    discovered.value = ["/tmp/initial.md"];
    await wrapper.vm.$nextTick();
    expect(wrapper.vm.panel.open.value).toBe(false);

    isBusy.value = true;
    discovered.value = ["/tmp/initial.md", "/tmp/streamed.md"];
    await wrapper.vm.$nextTick();
    expect(wrapper.vm.panel.open.value).toBe(true);
    ...
    defineComponent({
      setup() {
        const panel = useArtifactPanel(
          ...
          { autoOpen: computed(() => isBusy.value) },
        );
        return { panel };
      },
      render() {

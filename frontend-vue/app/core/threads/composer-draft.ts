...
export function writeComposerDraft(
  ...
  try {
    ...
    storage.setItem(key, JSON.stringify({ version: COMPOSER_DRAFT_VERSION, ...draft }));
  } catch {
    // Drafting remains usable when browser storage is unavailable.
  }
}
...
export function clearComposerDraft(
  ...
  try {
    ...
    // Sending remains usable when browser storage is unavailable.
  }
}

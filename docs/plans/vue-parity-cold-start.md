...
### D. ~~挂着的账~~ —— **三条 wave 101/102/103 全部处理完，这一段空了**

1. ~~**artifact 头部长文件名的 18px 零头**~~ —— **wave 103 复量完：不算差异，账结清**。
   用户可见的部分两边逐字相同（标题 `span` 407/407 被裁 0px、`SelectTrigger` 都是 455 宽、
   四颗动作键位置差 1px 且无越界）。**并订正那个读数本身**：wave 82 的
   「481/458 vs 497/492」是拿两个不同层级的盒子在比——两边 clientWidth 差 34px
   本身就是证据。**这三条账现在全部处理完了，D 段是空的。**

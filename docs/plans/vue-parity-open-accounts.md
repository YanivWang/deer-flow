# React → Vue 平替：挂账总清单（截至 wave 83，2026-09-05）
...
```
verify           exit 0    259 文件 / 2158 单测；词典 942 key / 18 unused
...
3. **把散文里的断言变成守卫**（`tests/guards/` 下已有九条 + wave 83 加进
   ...
   **wave 83 刚证明这条最值钱**：一张零消费者的表，17 条里 2 条是假的，
   而假话在报表上和真话逐字相同。同形的下一个目标：`baseline/*.json` 的
   `$semantics`、各文件头「实测过、做不到」的结论。**判据是「哪一行代码读它」。**

> **这条尾巴没有自然终点。** 历史命中率：wave 75 捞出 6 处真差异、wave 76 捞出 27 处、
> wave 82 捞出一个**两个应用都存在**的产品缺陷、wave 83 证伪了**验收判据自己**。**什么时候收，是个停止规则问题，
> 不是一个能算出来的轮数。**

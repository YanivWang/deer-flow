...
- 分支 `main-wc`。`b700cf17` = wave 39（chore `b09adb80`），
  ...
  `a1d675d6` = wave 51，`3382f7e0` = wave 52，`333edeef` = wave 53，`ff2cd759` = wave 54，`c8b2d1a8` = wave 55，`509219ea` = wave 56，`ccf6d0b8` = wave 57，`3e47b1fd` = wave 58。
- **动过 `frontend/` 的是十四轮，不是三轮**（wave 52 实测订正）：
  ...
  **边界规则本身有机器在守，需要人记的只有「这类改动做过哪些轮」。**
  ...
  可访问名；wave 36 `SidebarTrigger` 的窄屏图标。wave 41~58 都没动过。
...
产品 SFC **215**（总 217，wave 30~58 都没有新增 SFC）。动了 `frontend/` 再加
...
**用 wave 57 的新方法（先在本仓内部找自相矛盾的两句）做系统扫描，
另外正面查了「挂着的账」里还没复核过的几条。**

### 翻出来的：一条被「复验」过两次的理由，从头就是错的

`Button` as-child 那条账写着「wave 34 与 wave 37 **两次复验**：两边**都没有任何
选择器**消费它」。**而 `tests/unit/settings/settings-panels.dom.test.ts` 就写着
`button[data-slot="button"]`，还逐个断言 `data-variant`**——它落在 **wave 23**
（`3d6bb266`），**比那次「复验」早十四轮**；全仓另有 13 个测试文件在做同类断言。
**线索 152 第 N 次：「已核实」三个字本身要重验。**

**决定不变**（那条选择器要的是真 `<button>` 上的 `data-*`，本仓一直就有；
as-child 缺的只是两处 `<NuxtLink>` 上的），**错的是判据写太松**——已收紧成
两条 grep 的交集（见「挂着的账」那一条），**实测交集为空**。

### 扫过、没找到问题的

同名常量取值冲突（43 个常量，0 冲突）；注释里紧挨常量名却写另一个数（5 处同现，全benign）；
**同一处 `上游文件:行号` 被引用两次以上的 53 组，两处描述全部一致**；
`input-box.tsx:1328` vs 守卫头「实际在 1331」——**1331 今天仍然正确**，3 行漂移是设计容忍；
`scenarios.ts` 里不带日期的「两个应用都…」断言（台账本身就是它们的检查，抽查两条上游合同精确）；
A8 的「4 类缓存 / 6 个 key」与 1500ms 延迟两边一致。

---

## 下一轮（wave 59）：**收尾判据仍在重新计数**

47 干净，48~58 十一轮各翻出一条。别报「还剩几轮」。

wave 59 扫哪里：

1. **「挂着的账」里剩下的每一条都按 wave 58 的方式重验一遍**。wave 55 查了四条、
   wave 58 查了 as-child 这条，**还剩案例页四条请求、`/auth/callback`、首次发送那一屏、
   欢迎区位置、模型选择器筛选、上游种子取数失败、共有块死条目**没有逐条重验。
   **重点是判据本身写得够不够紧**——as-child 那条就是判据松到会被误触发。
2. **`tests/` 下所有 `describe`/`it` 的标题 vs 它实际断言的东西**。标题是下一个人
   决定「这条测试还需不需要」的唯一依据，而标题从来没被对过。
3. **`app/core/**` 与 `app/composables/**` 里的 `@link` / `{@link}` 引用**（TSDoc），
   wave 46/51/54 钉的是散文里的路径与行号，**TSDoc 的符号引用是另一套语法**，没查过。

## 更早几轮的记录（49~57 一句话，48 保留全文）

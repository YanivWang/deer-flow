...
- `begin_artifact_write` / `append_artifact_chunk` / `finalize_artifact_write` - Staged write protocol for long generated deliverables. Chunks default to 10 KB max via `DEERFLOW_ARTIFACT_CHUNK_MAX_BYTES`; finalization optionally verifies SHA-256 and, for HTML, complete document structure before the target path is written. `finalize_artifact_write` is subject to the read-before-write gate for existing targets.

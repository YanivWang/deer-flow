---
name: ppt-generation
description: 当用户请求生成、创建或制作 PPT/PPTX 演示文稿时使用此 skill。它为每页生成视觉图像并组合成 PowerPoint 文件。
---

# ppt-generation Skill（中文可替换运行版）

## 中文执行摘要

先规划演示结构和每页内容，再调用图像生成为各页创建视觉素材，最后用 Python 组装 PPTX。
...
注意：它会依赖 image-generation skill 产出页面图片，再组合 PPTX。

## 原理和运行流程

`ppt-generation` 的核心原理：先规划演示结构和每页内容，再调用图像生成为各页创建视觉素材，最后用 Python 组装 PPTX。
...
```mermaid
flowchart TD
  A["用户请求匹配 ppt-generation"] --> B["Agent 读取 SKILL.md"]
  B --> C["按正文工作流理解需求和输入材料"]
  C --> D{"是否需要额外资源"}

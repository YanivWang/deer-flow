...
e2e-parity       56        台账 0 行 / 48 样本（NEW=0 GONE=0）
...
1. **给取样面加交互态**（台账看不见的第①⑦类）。判据：一个域收工前，把它所有
   ...
   **还没接的交互态**（`integrations` 的权限面板 wave 88、`channels` 的连接对话框
   wave 90 都做掉了，剩三处）：`branch-thread`（分支入口在消息动作条上，
   那一排是 tooltip 触发，可能要先加一档 hover 步骤）、
   ...
   **wave 90 留下一条通用办法**：两边没有共用 testid 时，先去**夹具**里找字符串
   ——它不进词典、两种语言逐字相同，而且两个应用拿到的是同一份。
...
> **这条尾巴没有自然终点。** 历史命中率：wave 90 把挂了好几轮的 `channels` 对话框

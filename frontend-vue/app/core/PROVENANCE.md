...
| `markdown/rendering-context.ts`       | `ADAPTED` | `streamdown` animation context                                 | WP-13：以 Vue 注入契约传递消息流式状态，使表格与代码块操作在动画期间和 React Streamdown 一样禁用；不建立第二份 run 状态。                                                              |
...
| `messages/processing.ts`              | `ADAPTED` | `components/workspace/messages/message-group.tsx`              | WP-03：每组一次性关联 tool result/browser frame，投影较早步骤、最新工具、trailing reasoning 与其后答案的稳定布局。                                                                     |
| `models/capabilities.ts`              | `ADAPTED` | `components/workspace/input-box.tsx`                           | WP-03：把 agent 默认模型与 thinking mode 归一化抽成纯函数；capability 控制 UI，explicit effort 保留给 React 等价提交 context。                                                         |
| `threads/message-identity.ts`         | `ADAPTED` | `threads/hooks.ts`                                             | M4a 消息身份/去重；Gateway 历史行上的 run feedback 继续无损携带，但 React 当前未启用可见 feedback 操作，Vue 不据此增加按钮。                                                           |
...
| `threads/submit.ts`                   | `ADAPTED` | `threads/hooks.ts`                                             | M4a 合并两处 run context；WP-03 保持 React 的 explicit effort → mode fallback 请求语义，Gateway model factory 仍负责 provider capability 过滤。                                        |

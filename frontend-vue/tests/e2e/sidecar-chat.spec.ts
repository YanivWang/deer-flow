...
async function expectSidecarModelPinnedToSubmit(page: Page) {
  const metrics = await page.evaluate(() => {
    const getComposerMetrics = (placeholderPattern: RegExp) => {
      ...
      const modelElement = form.querySelector(
      ...
      const model = modelBox
        ? {
            ...
            left: Math.round(modelBox.left),
            right: Math.round(modelBox.right),
          }
        : buttons.find((button) => button.label === "DeepSeek V4 Pro");
...
test.describe("Side chat", () => {
  test("creates a hidden sidecar thread from selected quoted text", async ({
    ...
    await quoteAttachment.getByTestId("clear-conversation-references").click();

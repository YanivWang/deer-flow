...
import type { RichInlinePart } from "../../../core/messages/rich-content";
...
<template>
  <template v-for="(part, index) in parts" :key="`${part.type}-${index}`">
    ...
    <a
      ...
      :class="{ 'rich-message-content__streaming-reveal': part.reveal }"
      :data-testid="part.reveal ? 'vue-message-streaming-reveal' : 'vue-message-image-link'"
      :href="part.src"
      target="_blank"
      rel="noopener noreferrer"
    >
      <img :src="part.src" :alt="part.alt" loading="lazy" decoding="async">
    </a>
  </template>
</template>

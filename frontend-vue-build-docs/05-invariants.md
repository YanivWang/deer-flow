...
上面写「落在适配层」时还不知道 M1 会把它作为 `COPIED` 逐字节落进 `api/`。移动它的
代价是实测过的：codemod 只改 `@rstest/core → vitest` 与 `rs.* → vi.*`，**不改
`@/core/…` specifier**，所以移动会逼 `stream-mode.test.ts` 转 `HAND_MAINTAINED`，
外加 land-copied / eslint.config / .prettierignore 三处路径映射。而 A2 真正要保的
行为是「校验发生在适配层构造请求时、在 HTTP 之前」——那由
`agent-deerflow/run-protocol.ts` 的调用点决定，与文件放哪无关；「不在内核里」这条
已由 `tests/architecture.test.ts` 禁 `messages-tuple` 机器守住。
**不做 re-export**：给同一个符号造两条 import 路径比放在一个不那么直觉的目录更糟。

**2. 直接把 `sanitizeRunStreamOptions` 套在 wire 请求体上，校验什么都不做。**

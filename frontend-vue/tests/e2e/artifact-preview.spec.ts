...
test.describe("Artifact preview stability", () => {
  test("renders preview iframe for an in-progress write artifact", async ({
    ...
    await expect(page.getByRole("button", { name: ARTIFACT_PATH })).toBeVisible({
      timeout: 15_000,
    });
    await page.getByRole("button", { name: ARTIFACT_PATH }).click();

    const artifactsPanel = page.locator("#artifacts");
    await expect(artifactsPanel.getByTestId("vue-artifact-selected-filename")).toHaveText("report.html");
    await expect(
  ...
  test("renders markdown preview for an in-progress write artifact", async ({
    ...
    await expect(page.getByRole("button", { name: MARKDOWN_ARTIFACT_PATH })).toBeVisible({
      timeout: 15_000,
    });
    await page.getByRole("button", { name: MARKDOWN_ARTIFACT_PATH }).click();

    const artifactsPanel = page.locator("#artifacts");
    await expect(artifactsPanel.getByTestId("vue-artifact-selected-filename")).toHaveText("report.md");
  ...
  test("renders code view for an in-progress non-preview write artifact", async ({
    ...
    await expect(page.getByRole("button", { name: JSON_ARTIFACT_PATH })).toBeVisible({
      timeout: 15_000,
    });
    await page.getByRole("button", { name: JSON_ARTIFACT_PATH }).click();

    const artifactsPanel = page.locator("#artifacts");
    await expect(artifactsPanel.getByTestId("vue-artifact-selected-filename")).toHaveText("report.json");
  ...
  test("keeps an opened presented artifact in the header dropdown", async ({
    ...
    await expect(page.getByRole("button", { name: PRESENTED_ARTIFACT_PATH })).toBeVisible({
      timeout: 15_000,
    });
    await page.getByRole("button", { name: PRESENTED_ARTIFACT_PATH }).click();

    const artifactsPanel = page.locator("#artifacts");

    await expect(artifactsPanel.getByTestId("vue-artifact-selected-filename")).toHaveText("presented-report.md");
    await expect(artifactsPanel.getByText("Presented Report")).toBeVisible();

    const artifactSelect = artifactsPanel.getByRole("combobox");
    await artifactSelect.selectOption("/artifact-fixtures/report.md");
    await expect(artifactsPanel.getByText("Thread Report")).toBeVisible();

    await artifactSelect.selectOption(PRESENTED_ARTIFACT_PATH);
    await expect(artifactsPanel.getByText("Presented Report")).toBeVisible();
  });
  ...
  test("renders sandboxed iframe for a browser-previewable non-code file (urlOfArtifact path)", async ({
    ...
    await expect(page.getByRole("button", { name: PDF_ARTIFACT_PATH })).toBeVisible({
      timeout: 15_000,
    });
    await page.getByRole("button", { name: PDF_ARTIFACT_PATH }).click();

    const artifactsPanel = page.locator("#artifacts");
    await expect(artifactsPanel.getByTestId("vue-artifact-selected-filename")).toHaveText("report.pdf");
  ...
  test("renders video artifacts with a native media element instead of a sandboxed iframe", async ({
    ...
    await expect(page.getByRole("button", { name: VIDEO_ARTIFACT_PATH })).toBeVisible({
      timeout: 15_000,
    });
    ...
    const artifactsPanel = page.locator("#artifacts");
    await expect(artifactsPanel.getByTestId("vue-artifact-selected-filename")).toHaveText("dancer-video.mp4");

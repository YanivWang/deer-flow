...
description: 当用户请求对 GitHub 仓库或开源项目做多轮深度研究、时间线重建、竞品分析或深入调查时使用此 skill。
---

# github-deep-research Skill（中文可替换运行版）

## 中文执行摘要

通过 GitHub API、仓库资料和报告模板做多轮开源项目研究，产出时间线、指标、结构化分析和 Mermaid 图。
...
`github-deep-research` 的核心原理：通过 GitHub API、仓库资料和报告模板做多轮开源项目研究，产出时间线、指标、结构化分析和 Mermaid 图。
...
```mermaid
flowchart TD
  A["用户请求匹配 github-deep-research"] --> B["Agent 读取 SKILL.md"]
  B --> C["按正文工作流理解需求和输入材料"]
  C --> D{"是否需要额外资源"}

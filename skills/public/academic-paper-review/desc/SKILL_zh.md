...
description: 当用户请求审阅、分析、批判或总结学术论文、研究文章、预印本或科学出版物时使用此 skill。支持方法论评估、贡献评价、文献定位和建设性反馈。
---

# academic-paper-review Skill（中文可替换运行版）

## 中文执行摘要

把论文输入转成结构化同行评审流程：先识别论文来源和研究背景，再按方法、贡献、实验、局限、相关工作和改进建议输出可引用的学术评审。
...
`academic-paper-review` 的核心原理：把论文输入转成结构化同行评审流程：先识别论文来源和研究背景，再按方法、贡献、实验、局限、相关工作和改进建议输出可引用的学术评审。
...
```mermaid
flowchart TD
  A["用户请求匹配 academic-paper-review"] --> B["Agent 读取 SKILL.md"]
  B --> C["按正文工作流理解需求和输入材料"]
  C --> D{"是否需要额外资源"}

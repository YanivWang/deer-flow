---
name: surprise-me
description: 当用户说 surprise me、想要灵感、觉得无聊或请求意外创意展示时使用此 skill。它会动态组合其它已启用 skill。
---

# surprise-me Skill（中文可替换运行版）

## 中文执行摘要

先发现当前可用 skill，再根据用户上下文创意组合多个能力，形成一个意外但可执行的展示。
...
`surprise-me` 的核心原理：先发现当前可用 skill，再根据用户上下文创意组合多个能力，形成一个意外但可执行的展示。
...
```mermaid
flowchart TD
  A["用户请求匹配 surprise-me"] --> B["Agent 读取 SKILL.md"]
  B --> C["按正文工作流理解需求和输入材料"]
  C --> D{"是否需要额外资源"}

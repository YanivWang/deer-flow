...
> **2026-08-02 用户验收覆盖层**：本项目当前执行不得以 source-backed anchor、mock 通过或“功能基本可用”作为最终完成。所有域必须同时满足 11 号方案的 1:1 功能与交互对标（事件顺序、状态机、输入、路由、缓存、错误、取消、恢复均包括在内）、UI 还原度至少 98%、企业级组件/依赖边界和企业级 SCSS/token/mixin 架构；视觉差异必须用 React 冻结基线截图与可复现 diff 记录，低于 98% 的页面继续保持 Open Gap。

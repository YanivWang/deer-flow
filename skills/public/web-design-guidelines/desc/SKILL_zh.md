...
description: 当用户要求审查 UI、检查可访问性、审计设计、评估 UX 或按 Web Interface Guidelines 检查站点时使用此 skill。
metadata:
  author: vercel
  version: "1.0.0"
...
# web-design-guidelines Skill（中文可替换运行版）

## 中文执行摘要

按 Web Interface Guidelines 对 UI 文件做结构化审查，指出可访问性、布局、交互和质量问题。
...
注意：frontmatter 的 `metadata.argument-hint` 必须保留。

## 原理和运行流程

`web-design-guidelines` 的核心原理：按 Web Interface Guidelines 对 UI 文件做结构化审查，指出可访问性、布局、交互和质量问题。
...
```mermaid
flowchart TD
  A["用户请求匹配 web-design-guidelines"] --> B["Agent 读取 SKILL.md"]
  B --> C["按正文工作流理解需求和输入材料"]
  C --> D{"是否需要额外资源"}

...
不是手工维护的组件 allowlist。当前 checkout 共有 156 个 Vue SFC：

- `app/app.vue`、`app/components/**/*.vue`、`app/layouts/**/*.vue`、
  `app/pages/**/*.vue` 中 154 个产品 SFC 全部进入 AST source guard；
...
| 类别               | 处理规则                                                                                                       | 当前结果                                 |
| ------------------ | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| 用户可见产品文案   | 模板文本与 UI 脚本值必须来自唯一 `$i18n` owner                                                                 | 154 个产品 SFC 无核心英文硬编码          |

...
| 7   | **上游那个不会自愈的加载态**          | **wave 101 新挂，未定性** | `/workspace/chats/new` 提交首轮后，React 约 25% 概率停在 A 态，而 A 态里 `LoadMoreHistoryIndicator`（`frontend/src/components/workspace/messages/message-list.tsx`）的 `isHistoryLoading` 一直为 true，一颗「Loading...」**等满 90s 也不消失（5/5）**；Vue 同一份夹具 20/20 干净。**先定性再动手**：换 `backend` 复跑一遍看还在不在；卡住的 90 秒里 `GET /api/threads/{id}/messages/page` 到底发没发、回没回；上游那颗 sentinel 在一个全新线程上凭什么 `hasMore` 为真。**本仓不是「没挂那颗按钮」**——`MessageList.vue` 有逐字对齐的同一颗，渲染条件 `v-if="hasMoreHistory || historyLoadingMore || historyError"` 与上游 `if (!hasMore && !isLoading) return null` 等价。所以问题要问得更准：**为什么上游的 `isHistoryLoading` 会在一个全新线程上变成 true 并一直不落，而本仓的 `historyLoadingMore` 不会**。定案属上游缺陷则按 fork-boundary 两边同改。 |
...
| 2   | **覆盖率棘轮的 pending 1 条**         | **开着；wave 101 量到底，判据未满足** | `chat-thread-init-ordering`。**wave 101 按登记判据连取 20 次：React 仍是两个终态 15 B / 5 A（25%），Vue 20/20 单一——判据「只出现一个终态」未满足，`pending` 保留**（不许改判据凑绿，硬规则 3）。**同一轮订正了病因**：原文写「A 只是慢、几秒会收敛」是错的，取样后再等 90s，**5 个 A 样本无一收敛**；wave 62 的「14/14 都清掉了」只说明那 14 次没出现 A。所以挡路的不是取样抖，是下面第 7 条那个加载态。A 率 17%~45% 之间摆，**wave 63 与 wave 101 之差不显著（Fisher 双侧 p=0.094），别当回归**。读数、哈希与复现方法逐字写在 `baseline/parity-scenario-coverage.json` 的 `$pendingReasons`。 |

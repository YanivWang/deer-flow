...
## 四、收工时的门禁读数（wave 95 逐条实跑）

```
verify           exit 0    262 文件 / 2180 单测；词典 942 key / 18 unused
                           产品 SFC 218（总 220）
standalone-sim   exit 0    跑过 13 / 未跑 5（4 data + 1 e2e）/ 红 0      ← wave 83 新增
                           --with-e2e 实测 13 / 4 / 0（那一条：exit 0，47 条跳过）
e2e-parity       81        台账 51 行 / 73 样本（三类都见第一节第 3、4、5 条）
...
> **这条尾巴没有自然终点。** 历史命中率：wave 95 把第④类的顺序那一半补上、

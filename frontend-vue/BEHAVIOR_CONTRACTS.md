...
| S8b | 事件库为空时退回最新 checkpoint 走 **`POST /history`**（`{ limit: 1 }`），不是 `GET /state`：后端只有 `get_thread_history` 会把 run 身份写回消息（每条 ai/tool 的 `run_id` + 每回合最后一条 AI 的 `turn_duration`），`get_thread_state` 两个字段都没有。React 侧 LangGraph SDK 的 `fetchStateHistory: { limit: 1 }` 走的也是这一条。**checkpoint 消息没有 run 身份时留空串，不得凭空造一个**——造出来的 id 会被 `WorkspaceChangesBadge` 当成真 run 发出注定 404 的请求。      |

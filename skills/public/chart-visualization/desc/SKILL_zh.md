...
description: 当用户希望把数据可视化时使用此 skill。它会从多种图表类型中选择合适图表，提取参数，并调用 JavaScript 脚本生成图表图像。
compatibility:
  nodejs: ">=18.0.0"
---

# chart-visualization Skill（中文可替换运行版）

## 中文执行摘要

先根据数据和表达目标选择图表类型，再读取对应 reference 规范提取参数，最后调用 `scripts/generate.js` 输出图表图像。
...
注意：frontmatter 的 `compatibility.nodejs` 必须保留；实际图表输出依赖 Node.js 脚本。

## 原理和运行流程

`chart-visualization` 的核心原理：先根据数据和表达目标选择图表类型，再读取对应 reference 规范提取参数，最后调用 `scripts/generate.js` 输出图表图像。
...
```mermaid
flowchart TD
  A["用户请求匹配 chart-visualization"] --> B["Agent 读取 SKILL.md"]
  B --> C["按正文工作流理解需求和输入材料"]
  C --> D{"是否需要额外资源"}

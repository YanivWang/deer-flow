...
description: 当用户需要跨多篇论文做系统综述、survey、annotated bibliography 或交叉论文比较时使用此 skill。
---

# systematic-literature-review Skill（中文可替换运行版）

## 中文执行摘要

围绕研究问题搜索 arXiv，多篇论文筛选、归纳和比较后，按 APA/IEEE/BibTeX 等格式输出系统综述。
...
`systematic-literature-review` 的核心原理：围绕研究问题搜索 arXiv，多篇论文筛选、归纳和比较后，按 APA/IEEE/BibTeX 等格式输出系统综述。
...
```mermaid
flowchart TD
  A["用户请求匹配 systematic-literature-review"] --> B["Agent 读取 SKILL.md"]
  B --> C["按正文工作流理解需求和输入材料"]
  C --> D{"是否需要额外资源"}

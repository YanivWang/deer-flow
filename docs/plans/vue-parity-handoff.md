...
- **对照台账 107 行**，**75** 个样本，`make -C frontend-vue e2e-parity` **83** 条全绿（wave 128：95/73/81 → 107/75/83，新增 12 行逐条有名有姓）。
...
⑥ 只在某种后端状态下才分叉的渲染路径——**wave 128 第一次接上**（`ParityState.routes` + `integrations#load-failed`），第一跑就量出上游重试 3 次而本仓 1 次；⑦ **这一屏压根没被取样**；

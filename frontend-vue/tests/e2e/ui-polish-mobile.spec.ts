...
test.describe("UI polish mobile regressions", () => {
  ...
  test("mobile artifacts open in a drawer without horizontal overflow", async ({
    ...
    await expect(page.getByTestId("artifact-trigger")).toBeEnabled({ timeout: 15_000 });
    await page.getByTestId("artifact-trigger").click();

    await expect(page.getByTestId("vue-artifact-panel")).toBeVisible();
    await expect(page.getByTestId("vue-artifact-selected-filename")).toHaveText(
      "mobile-summary.md",
    );
    await expect
      .poll(() => page.evaluate(() => document.documentElement.scrollWidth))

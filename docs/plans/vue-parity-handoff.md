...
- 分支 `main-wc`。`b700cf17` = wave 39（chore `b09adb80`），
  ...
  `a1d675d6` = wave 51，`3382f7e0` = wave 52，`333edeef` = wave 53，`ff2cd759` = wave 54，`c8b2d1a8` = wave 55。
- **动过 `frontend/` 的是十四轮，不是三轮**（wave 52 实测订正）：
  ...
  **边界规则本身有机器在守，需要人记的只有「这类改动做过哪些轮」。**
  ...
  可访问名；wave 36 `SidebarTrigger` 的窄屏图标。wave 41~55 都没动过。
...
产品 SFC **215**（总 217，wave 30~55 都没有新增 SFC）。动了 `frontend/` 再加
...
**按 wave 54 指的方向：把所有「记错了 / 已翻案 / 已做完」的条目拉出来，
逐条问「那处真的改了吗」，外加从没审过的 `packages/agent-core/`。**

### 翻出来的：一条记录比被记录的东西落后十六轮

本文件的 browser 面板那条账最后写着「`BrowserPanel.vue` 的文件头已同步改过
（**四处 → 三处**）」，而文件头写的是**两处**——`b700cf17`（**wave 39**）两边同改
补上关闭按钮的可访问名时，文件从三处改成两处，**这里没跟**。
比 wave 54 那条轻，但**是同一类**（线索 180）。已改成写清剩下哪两处，
而不是留一个会继续烂的数字。

### 顺手补上一个真存在的单向断言

`packages/agent-core/tests/contract.test.ts` 的「错误 kind 成员和数量都对得上」
此前只从**源码**一侧看，**文档多点名一个源码里没有的 kind 会全绿**。已补反方向。
**这个洞是量出来的**：第一条负向变异（把文档里的 `replay_gap` 改成 `replay_gone`）
**收紧前就已经 RED**（旧断言接住了）；换成「真名一个不少、数量词不动，只多加一个」
才复现 GREEN→RED。**声称补了覆盖之前，先证明旧的确实没覆盖**（线索 170 的反用）。

### 逐条复核、全部成立的

`settings.memory.*` 六条只剩有意留的 `rawJson`；wave 33 删的十条死词条一条没回来；
`failedSend` 的判据注释还在，它引的 `hooks.ts:2390` **逐字**就是那句 `toast.error(...)`；
`isMock` 上游仍是 23（`hooks.ts` 内，全仓 119）；四条「上游也零消费」全部成立——
**`conversation.startConversation` 的机制比记的更细**：上游把那句话写死在
`ai-elements/conversation.tsx:45` 的默认值里（与 N6 同类），而**三个调用点全都自己传了
title/description**，默认值一次都没用上，所以本仓不移植默认文案没有造成 UI 缺口。
agent-core 的 `files: ["src"]`、单一 `exports`、`RETRYABLE_KINDS` 只含 `network`、
`reconnects` 只增不减——全部成立。

### 量过之后决定不做的：给本文件上门禁

本文件在所有守卫的扫描面之外（`git ls-files` 只列 `frontend-vue/`）。实测它有
**28 处反引号仓库路径 + 25 处裸文件名**，6 处解析不到——**而那 6 处全是有意引用
一个已死的东西**（wave 51/54 自己记的死引用、每轮临时建又删掉的 `probe.spec.ts`）。
上门禁就得把它们逐条圈进历史区，**豁免会比门禁长**。
它真正会烂的是**计数**，而计数没法靠正则通用地钉住。**如实记下它没有门禁。**

---

## 下一轮（wave 56）：**收尾判据仍在重新计数**

47 干净，48~55 八轮各翻出一条（55 那条最轻：文档里的一个旧计数）。别报「还剩几轮」。

wave 56 扫哪里：

1. **本文件里的所有计数**。wave 52（三次 vs 十四轮）与 wave 55（三处 vs 两处）
   都是计数烂掉。**把本文件里每一个「N 处 / N 条 / N 个 / N 轮」拉出来，逐条量。**
   这是已知最脏的一类，而且没有门禁。

...
| 复用 vs 重写 | 🔴 **总表在 [§1.0](11-vue-parity-plan.md)，实现前先看这张**（此前散在 6 个章节）。四档：①逐字节复制 **18,747 行**（依赖清单见 §2.8.3，Vue manifest 为 **50 个直接依赖**）②改几行即可 **3,268 行** ③必须重写 **≈29,680 行 + 4 个自研基础件 + 4 个缺口件 + 37 个耦合包的替代** ④直接不做 **7,025 行 + 72 MDX**<br>继承占总工作面 **42.6%** |
...
| 验收定义 | 🔴 **分两层（D14）**：① UI 层 24 个 E2E spec 全绿 ② 代理层 `proxy-policy` 6 条断言 + 4 个真后端 spec 全绿。<br>**只签①等于 Nitro 代理层零验证** —— 24 spec 走 `page.route()`，从未执行过代理 |

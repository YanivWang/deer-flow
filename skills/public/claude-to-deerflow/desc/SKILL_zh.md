...
description: 通过 DeerFlow HTTP API 与 DeerFlow AI Agent 平台交互。当用户想向 DeerFlow 发送消息、启动线程、检查状态、管理记忆、上传文件或委托研究任务时使用。
---

# claude-to-deerflow Skill（中文可替换运行版）

## 中文执行摘要

把用户意图映射到 DeerFlow HTTP API 操作，并通过 `status.sh` / `chat.sh` 等脚本检查服务、创建会话、发送消息或委托任务。
...
`claude-to-deerflow` 的核心原理：把用户意图映射到 DeerFlow HTTP API 操作，并通过 `status.sh` / `chat.sh` 等脚本检查服务、创建会话、发送消息或委托任务。
...
```mermaid
flowchart TD
  A["用户请求匹配 claude-to-deerflow"] --> B["Agent 读取 SKILL.md"]
  B --> C["按正文工作流理解需求和输入材料"]
  C --> D{"是否需要额外资源"}

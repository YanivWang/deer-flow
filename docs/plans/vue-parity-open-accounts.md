# React → Vue 平替：挂账总清单（截至 wave 84，2026-09-05）
...
| 3   | **两处「只能变短」没有任何机器在守**   | **开着（wave 84 量出）** | `baseline/parity-diff.json` 的 `$comment` 与 `baseline/parity-scenario-coverage.json` 的 `$semantics` 各写了一遍「这份清单只能缩短 / pending 只能变短」，`tests/e2e-parity/support/scenarios.ts:561` 是第三份。实测**没有任何检查在守这个方向**：`make parity-accept` 与手改 baseline 都能让它变长而全绿。**先决定它是承诺还是评审政策**——真要当承诺，可行的判据是「不许有 id 从 covered/exempt 退回 pending」（不依赖 git 历史也算得出来）；当政策就把那三句话改成准确的说法。 |
...
```
verify           exit 0    259 文件 / 2159 单测；词典 942 key / 18 unused
...
> wave 84 捞出三条「扫描器盖不全自己声称的范围」。**什么时候收，是个停止规则问题，
> 不是一个能算出来的轮数。**

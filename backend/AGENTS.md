...
│   │           │   ├── tools.py       # bash, ls, read/write/str_replace, staged artifact writes
...
- `write_file` - Write/append small files, creates directories; overwrites by default and exposes the `append` argument in the model-facing schema for short end-of-file writes; subject to the read-before-write gate when `read_before_write.enabled` (see Middleware Chain)
...
- `begin_artifact_write` / `append_artifact_chunk` / `finalize_artifact_write` - Staged write protocol for long generated deliverables. Chunks default to 10 KB max via `DEERFLOW_ARTIFACT_CHUNK_MAX_BYTES`; finalization optionally verifies SHA-256 and, for HTML, validates complete document structure plus local resources referenced by `src`, `srcset`, stylesheet/icon `href`, and CSS `url(...)` before the target path is written. `finalize_artifact_write` is subject to the read-before-write gate for existing targets.

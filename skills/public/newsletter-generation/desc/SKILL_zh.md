...
description: 当用户请求生成、撰写或起草 newsletter、邮件摘要、周报、行业简报或 curated content summary 时使用此 skill。
---

# newsletter-generation Skill（中文可替换运行版）

## 中文执行摘要

先研究主题和受众，再筛选来源、组织栏目和摘要，最后生成适合邮件或网页分发的 newsletter。
...
`newsletter-generation` 的核心原理：先研究主题和受众，再筛选来源、组织栏目和摘要，最后生成适合邮件或网页分发的 newsletter。
...
```mermaid
flowchart TD
  A["用户请求匹配 newsletter-generation"] --> B["Agent 读取 SKILL.md"]
  B --> C["按正文工作流理解需求和输入材料"]
  C --> D{"是否需要额外资源"}

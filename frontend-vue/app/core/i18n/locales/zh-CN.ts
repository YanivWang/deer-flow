/*
  【文件职责】     见下方源码；本文件由 frontend/src/core/i18n/locales/zh-CN.ts retype 而来。
  【对应 frontend/】 frontend/src/core/i18n/locales/zh-CN.ts
  【架构位置】     L3
  【主要导出】     zhCN
  【依赖关系】     见下方 import；改写清单由 scripts/land-retyped.mjs 声明
  【边界与注意】   RETYPED：内容**不是**上游逐字节等同，因此不参与 COPIED hash 护城河。
                   相对上游的改动只有这些：图标包换 Vue 版；LucideIcon 与所用图标名在 lucide-vue-next 中同名（已实测）。（lucide-react → lucide-vue-next）
...
export const zhCN: Translations = {
  ...
  scheduledTasks: {
    ...
    detail: {
      ...
      loadingMore: "正在加载更多…",
      runCount: "运行次数",
      runningLocked: "该任务正在运行；当前运行结束后才能执行冲突操作。",
    },
    actions: {
      edit: "编辑",
      cancelEdit: "取消编辑",

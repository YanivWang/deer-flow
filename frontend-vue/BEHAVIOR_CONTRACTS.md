...
| B23 | assistant 尾部动作统一由 `AssistantTurnActions` 使用共享 `ghost/icon-sm` Button 渲染：按钮 32 px、相邻步进 36 px、行内 gap 4 px、动作到耗时 8 px；消息滚动区使用 `stable both-edges` gutter，尾部由 32 px flex gap + 24 px spacer + 16 px padding 形成 72 px 结构 | 裸图标按钮和单侧 scrollbar gutter 会同时造成按钮命中区、横向起点与尾部间距偏离 React；这些尺寸必须由组件和列表结构拥有，而不是页面偏移补丁 |

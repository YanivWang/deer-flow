...
export function mockLangGraphAPI(page: Page, options?: MockAPIOptions) {
  ...
  void page.route("**/api/threads/*/artifacts/**", (route) => {
    ...
    const filename = pathname.split("/").at(-1) ?? "artifact.txt";
    if (filename.endsWith(".html")) {
      return route.fulfill({
        status: 200,
        contentType: "text/html",
        body: "<!doctype html><html><body><h1>Report draft</h1><p>测试内容</p></body></html>",
      });
    }
    if (filename.endsWith(".md") || filename.endsWith(".mdx") || filename.endsWith(".skill")) {
      return route.fulfill({
        status: 200,
    ...
    if (filename.endsWith(".json")) {
      return route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ status: "draft", "中文字段": "测试内容" }, null, 2),
      });
    }
    ...
    if (filename.endsWith(".mp4")) {
      return route.fulfill({ status: 200, contentType: "video/mp4", body: "video-fixture" });
    }
    return route.fulfill({ status: 200, contentType: "text/plain", body: "artifact content" });
  });

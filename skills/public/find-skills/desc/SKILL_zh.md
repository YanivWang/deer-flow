---
name: find-skills
description: 当用户询问如何获得某种能力、寻找可安装 skill 或表达想扩展能力时使用此 skill。
---

# find-skills Skill（中文可替换运行版）

## 中文执行摘要

先判断用户想扩展哪类能力，再搜索/推荐/安装合适 skill，并用安装脚本完成落地。
...
`find-skills` 的核心原理：先判断用户想扩展哪类能力，再搜索/推荐/安装合适 skill，并用安装脚本完成落地。
...
```mermaid
flowchart TD
  A["用户请求匹配 find-skills"] --> B["Agent 读取 SKILL.md"]
  B --> C["按正文工作流理解需求和输入材料"]
  C --> D{"是否需要额外资源"}

...
> 状态快照：2026-08-13。本文是续接 `frontend-vue` 时唯一的当前状态入口。历史 evidence
...
- 生产入口仍是 **React default / Vue secondary hostname**。公网 Vue hostname、DNS/TLS、
  外层 LB/CDN、真实 IdP 和目标 runtime 尚无可用目标资源，故公开激活状态是 **BLOCKED**，
  不是 GO。
...
| `make e2e-m7` | **120/120 × 3 连续** | `23:46:56`、`23:47:46`、`23:48:42` 开始；0 retries |
...
React 侧回归：修改后的 artifact preview + sidecar **17/17**；React 自有 batched +
resize/transition **7/7**；real-backend multi-run **1/1**。
...
- 因无实际域名/endpoint/账号，DNS、TLS/SNI/证书链、HTTPS redirect、LB Host、外层 SSE
  ...
  browser 长运行均为 BLOCKED/UNRUN。

目标资源到位后，按 evidence 的命令验证，并用用户 Chrome 的现有登录态完成双 hostname
登录/回跳/刷新/登出；fixture、replay、localhost、自签名或无登录浏览器不能替代。
...
仓库内没有剩余必做迁移任务。下一项只能是外部目标环境激活；缺少实际 Vue hostname、TLS/LB、
IdP client、部署 URL 和测试账号时保持 BLOCKED，不能继续用仓库 fixture“补证明”。

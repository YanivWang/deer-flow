/*
  ...
  【对应 frontend/】 React settings e2e 无同等真实 Gateway 对照
...
const APP = `http://localhost:${process.env.E2E_WP10_FRONTEND_PORT ?? "3113"}`;
const GATEWAY_PORT = process.env.E2E_WP10_GATEWAY_PORT ?? "8016";
const UNSUPPORTED_GATEWAY = `http://127.0.0.1:${process.env.E2E_WP10_UNSUPPORTED_GATEWAY_PORT ?? "8017"}`;
const SETTINGS_HOME_MARKER = resolve(
  `test-results/wp10-real-backend-${GATEWAY_PORT}-home.txt`,
);

/*
  【文件职责】     见下方源码；本文件由 frontend/src/core/api/api-client.ts retype 而来。
  【对应 frontend/】 frontend/src/core/api/api-client.ts
  【架构位置】     L3
  【主要导出】     StreamReplayGapData / StreamReplayGapError / isInactiveRunStreamError / isRunNotCancellableError / clearReconnectRun / getAPIClient
  【依赖关系】     见下方 import；改写清单由 scripts/land-retyped.mjs 声明
  【边界与注意】   RETYPED：内容**不是**上游逐字节等同，因此不参与 COPIED hash 护城河。
                   相对上游的改动只有这些：依赖不迁的模块（static-mode.ts、threads/static-demo.ts），该 import 必须删除或改写。删掉 static demo client 分支（01-scope 已排除静态模式），保留真实 LangGraph client。
...
function createCompatibleClient(isMock?: boolean): LangGraphClient {
  ...
  client.runs.stream = async function* (threadId, assistantId, payload) {
    ...
    const initialStream = originalRunStream(threadId as string, assistantId, {
      ...sanitizedPayload,
      onRunCreated(meta) {
        runId = meta.run_id;

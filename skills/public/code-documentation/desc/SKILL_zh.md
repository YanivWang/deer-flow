...
description: 当用户请求生成、创建或改进代码、API、库、仓库或软件项目文档时使用此 skill。支持 README、API 文档、架构文档、变更日志和开发者指南。
---

# code-documentation Skill（中文可替换运行版）

## 中文执行摘要

先理解代码库和目标读者，再选择 README、API、架构、注释或 changelog 等文档形态，最后输出可维护的工程文档。
...
`code-documentation` 的核心原理：先理解代码库和目标读者，再选择 README、API、架构、注释或 changelog 等文档形态，最后输出可维护的工程文档。
...
```mermaid
flowchart TD
  A["用户请求匹配 code-documentation"] --> B["Agent 读取 SKILL.md"]
  B --> C["按正文工作流理解需求和输入材料"]
  C --> D{"是否需要额外资源"}

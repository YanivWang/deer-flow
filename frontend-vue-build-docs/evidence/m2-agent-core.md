...
> **历史证据。** 本文记录 M2 关闭时的状态；“没有 UI 调用方”等红项已被 M4a 部分解决，
> 但不代表 M4b 产品 UI 已完成。续接任务以
...
# 第 4 个窗口：把 golden 覆盖从 7 种事件提到 9 种（真实录制，不是合成）

上一节红项第 1 条是「golden trace 只覆盖 7 种事件」。本窗口关掉了其中两种，
关法不是重新录一次 LLM。

## 关键发现：`checkpoints` / `tasks` 是**请求模式**，不是场景产物

它们不需要新的录制场景——同一份 replay fixture 换一个 `stream_mode` 就会产出。
上一窗口把它们和 `subagent namespace` / `reasoning` 一起归进「要换录制场景」，
**那是判断错误**：后两者确实要换场景（提示词明确禁止委派、replay 模型不出 reasoning），
前两者只要改 `run-protocol.spec.ts` 的一行。

代价只有一处，而且是**已经被 spec 写好提示的**：`--queue-maxsize` 要重调。

| stream_mode                  | 总事件数 | queue-maxsize | 结果                                     |
| ---------------------------- | -------- | ------------- | ---------------------------------------- |
| 原有 4 种                    | 74       | 32            | ✅                                       |
| + `checkpoints` `tasks`      | 226      | 32            | ❌ create 流自己被 gap（live burst 溢出） |
| + `checkpoints` `tasks`      | 226      | **128**       | ✅ **采用**                              |
| + 再加 `debug`               | 378      | 128           | ❌ 同上                                  |
| + 再加 `debug`               | 378      | 384           | ❌ 首个游标不再被逐出，gap 测不出来      |
| + 再加 `debug`               | 378      | 256           | ✅（未采用，见下）                       |

这个窗口两侧都有边界：**大于 live burst、小于总事件数**。spec 的两条失败消息
分别指出破在哪一侧，照着调即可——不需要猜。

## `debug` 有意不录

不是因为它大（虽然它一个人把 fixture 从 1.1MB 顶到 2.1MB）。是因为
**08 §402 的 raw-trace 覆盖清单里没有它**——那份清单写的是
`updates`/`custom`/`checkpoints`/`tasks`。`debug` 只出现在 §349 的 event-map 清单里，
那是「事件表必须显式覆盖」的要求，合成用例就能满足。

实测数据留在 `playwright.m0-real-backend.config.ts` 的注释里，想录随时能录。

## 副作用：逐字节测试必须缩范围，而且原因值得记

fixture 从 53KB 涨到 1.1MB 之后，`golden-trace.test.ts` 的「逐字节喂进去」**超时了**
（2.1MB 版本跑 42 秒没跑完）。原因不是数据量线性增长——**逐字节的代价是超线性的**：
每来一个字节都要在累积 buffer 上找一次分隔符，而 `checkpoints` 单帧就有约 16KB。

改法：逐字节只跑**前 64KB**（含 6 种事件的完整帧，也含第一个多字节字符——
实测在第 28,352 字节），整份改用 **997 字节**定长切。质数长度保证切点不与任何帧长
对齐，分隔符与多字节字符照样被切开，读的次数从百万级降到千级。跑完 1.7 秒。

> 这条顺带暴露了一个**性能事实**：分帧层在大帧上是 O(n²)。它不是缺陷（05 L6 的
> buffer 上限本来就会兜底），但值得记下来——真实后端如果开始发大帧，
> 逐字节到达的场景会明显变慢。**没有做任何优化，也没有测过真实网络下的影响。**

## 换掉录制之后，reducer 一行没改就全绿

226 帧重放跑完，「最终消息集合逐字段等于最后一帧 `values`」「3 条幽灵消息被清掉」
「`X` → `X__user` 顶替」「全程没有未知事件」四条断言**一个字都没改**就通过了。
`checkpoints` / `tasks` 各 52 / 100 帧落进 `KNOWN_IGNORED`，durable state 不受影响——
这正是它们该有的行为，现在有真实录制作证，不再是合成用例。

## 复跑

```bash
cd frontend-vue
make run-protocol-smoke   # 重录，产物在 test-results/real-backend/
make verify && make e2e-m0
```

实测：`verify` **exit 0**（84 文件 / 844 用例）· `migration-check` **exit 0** ·
`e2e-m0` **exit 0**（7 个子套件 / 14 用例）。fixture 1.1MB（gzip 后 18KB）。

## 红项更新

- 上一节红项 1 **范围收窄**：`checkpoints` / `tasks` 已由真实录制覆盖。
  仍未被 golden 覆盖的是 **`custom`**（已在请求里，这个场景就是不发）、
  **`debug`**（有意不录）、**subagent namespace**、**reasoning**。
  后两者要换录制场景，代价是新增一份 backend replay fixture——**没有做**。
- **新增**：分帧层在大帧上的 O(n²) 行为（见上），未优化、未在真实网络下测过。
- 其余红项原样有效。
